...
The most obvious way to get parameters from a request is to grab them directly. Snap has a few facilities to enable this.

Here is a set of functions that will grab a single parameter:
...
and a set of functions that will grab the full set of parameters:
...
In a handler we can use these functions as such:

```haskell
myHandler :: Handler b v ()
myHandler
